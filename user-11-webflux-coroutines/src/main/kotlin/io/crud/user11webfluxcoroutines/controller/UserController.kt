...
class UserController(
    ...
    @GetMapping("/{id}")
    suspend fun get(@PathVariable("id") id: Long): ResponseEntity<UserResponse> {
        val user = userService.findById(id)
        return ResponseEntity.ok(user.toResponse())
    }

    @GetMapping
    suspend fun list(): ResponseEntity<Flow<UserResponse>> {
    ...
    @DeleteMapping("/{id}")
    suspend fun delete(@PathVariable("id") id: Long): ResponseEntity<Void> {
    ...
    @PutMapping("/{id}")
    suspend fun update(@PathVariable("id") id: Long, @Valid @RequestBody userRequest: UserRequest): ResponseEntity<UserResponse> {
        val user = userService.findById(id)
