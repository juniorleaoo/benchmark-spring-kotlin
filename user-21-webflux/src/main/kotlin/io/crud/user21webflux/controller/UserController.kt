...
import io.crud.user21webflux.service.UserService
...
class UserController(
    ...
    @GetMapping("/{id}")
    fun get(@PathVariable("id") id: Long): Mono<ResponseEntity<UserResponse>> {
    ...
    @DeleteMapping("/{id}")
    fun delete(@PathVariable("id") id: Long): Mono<ResponseEntity<Void>> {
    ...
    fun create(@Valid @RequestBody userRequest: UserRequest): Mono<ResponseEntity<UserResponse>> {
        return userService.save(userRequest.toUser())
            .map {
                ResponseEntity.created(URI.create("/users/${it.id}"))
                    .body(it.toResponse())
            }
    }
